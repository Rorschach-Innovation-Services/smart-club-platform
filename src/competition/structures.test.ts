...
const WEEKLY_B1 = { blockIndex: 0, cadence: { kind: 'weekly' as const } };
const WEEKLY_B2 = { blockIndex: 1, cadence: { kind: 'weekly' as const } };
...
describe('EMCU divisions — same structure, different cadence', () => {
  ...
  it('Division 3 Stream 2 / Division 4 — every two weeks', () => {
    const m = ready(
      materialiseStage({
        stage: flat('div4', { blockIndex: 0, cadence: { kind: 'every-n-weeks', n: 2 } }),
        calendar: CAL,
        context: { registered: team(8) },
      }),
    );
  ...
  it('Division 5 — Saturdays only', () => {
    const m = ready(
      materialiseStage({
        stage: flat('div5', { blockIndex: 0, cadence: { kind: 'weekdays', days: [6] } }),
        calendar: CAL,
  ...
  it('catches a bi-weekly division that overflows its block', () => {
    const m = materialiseStage({
      stage: flat('div4-big', { blockIndex: 0, cadence: { kind: 'every-n-weeks', n: 2 } }),
      calendar: CAL,
