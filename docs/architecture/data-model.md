...
| Player registration | `TENANT#<t>#CLUB#<id>`   | `PLAYER#<naturalKey>` | —                                        | Queried by club pk; count drives `club.players` (denormalized onto the club as `playerCount`). `<naturalKey>` (e.g. ID/cell) gives dedup.                                                                                            |
...
| 4   | Update a club (affiliation, CQI, exco, docs, paid) | `UpdateItem … ConditionExpression version = :v`                               |
...
| 11  | Erase a tenant                                     | Patterns 2/5/7/10 + the config item — query each partition, then batch-delete |

No pattern requires a `Scan` — erasure (pattern 11) enumerates each known partition/index and
...
- **Audit:** `changedBy`/`changedAt` on the club record track `paid` and affiliation changes
  (compliance requirement).
- **Server validation (enforced):** club-name uniqueness per tenant (onboard + bulk), league
