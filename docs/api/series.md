...
| `POST /season-runs/:id/rebase`                  | admin | Adopt the live structure version. Body `{ structureVersion, version }` (both required, `400` otherwise). `409 "season run changed; refetch"` on a stale run `version`; `409` with `code: "structure_changed"` when the live structure is no longer the `structureVersion` the admin reviewed; `404` when the structure is gone. |
...
| `400`  | Any of these: the template is missing or unknown; `seasonLabel` is blank; the league is unknown; the calendar has the wrong shape or an unknown id; a date is not strict `YYYY-MM-DD` or the end is before the start (`code: "invalid_dates"`); `placement` doesn't give each stage an integer inside the calendar's blocks (`code: "bad_placement"`); `matchFormat` is malformed; or a config validator rejects the result (e.g. the 20-calendar limit). |
| `403`  | The caller is not an admin.                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `409`  | The league already has a competition on that calendar (`code: "competition_exists"`; start the season from that competition through the normal path), or a run with the same `leagueKey` + `seasonLabel` already exists (`code: "season_exists"`).                                                                                                                                                                                                        |
| `500`  | The config write succeeded but the run write failed (`code: "run_not_started"`, plus `competitionId`; see below).                                                                                                                                                                                                                                                                                                                                         |
...
tenant's clubs for the run's league, minus the competition's `excludeTeamIds`, then
`materialiseRun`, which applies pairing overrides, confirmed groups, pool qualifiers and
...
2. **Series.** A new group series is created exactly as `POST /series` creates one, so it is
   ...
   reverts a name the admin chose.
...
| `409`  | `"season run changed; refetch"`: `version` doesn't match (checked before anything is written). `"stage is awaiting entrants"` (`code: "awaiting_entrants"`, plus `reason`): no confirmed or derivable entrants yet. `code: "does_not_fit"`: a group's rounds overrun its block. `code: "no_block"`: the stage's block no longer exists on the run's calendar. `code: "competition_unbound"`: the run's competition is no longer bound to its league. `code: "released_overwrite"`: see step 1. `code: "venue_clash"`: the in-season clash gate refused a released series, with the same body `PATCH /series/:id` returns. `"series changed; refetch"`: a series moved under the write. |
